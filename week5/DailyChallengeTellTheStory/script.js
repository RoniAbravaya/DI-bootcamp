...
document.getElementById('shuffle-button').addEventListener('click', function () {
    // Shuffle the story at least three times
    for (let i = 0; i < 3; i++) {
        generateStory();
    }
});
...
function generateStory() {
    ...
    if (noun && adjective && person && verb && place) {
        ...
        var stories = [
            ...
            `In a magical land, ${person} found a mysterious ${noun} that was surprisingly ${adjective}. ${person} decided to ${verb} near ${place}.`,
            `${person} embarked on a thrilling journey to discover the hidden ${adjective} ${noun} in ${place}. Along the way, ${person} learned to ${verb} like a pro.`,
